...
class AppConfig:
    ...
    def __init__(self):
        ...
        # 数据存储路径
        self.record_audio_dir = os.path.join(
            self.base_dir, PathConstants.RECORD_AUDIO_DIR
        )
        self.video_frames_dir = os.path.join(
            self.base_dir, PathConstants.VIDEO_FRAMES_DIR
        )
        self.video_recordings_dir = os.path.join(
            self.base_dir, PathConstants.VIDEO_RECORDINGS_DIR
        )

        # 异步任务性能配置
        self.enable_performance_logging = True  # 是否启用性能日志
        self.enable_queue_monitoring = True  # 是否启用队列监控
